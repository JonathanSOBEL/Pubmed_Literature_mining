...
search_topic <- 'circadian [and] modelling' #'molecular [AND] Clock'
...
pub_dat=get_pubmed_data(search_topic,year_start=2010,year_stop=2015)

autors_table_clean=pub_dat[[1]]
pubmed_data=pub_dat[[2]]
########

#save(pubmed_data,file="pubmed_data_Molecular_clock_1970_2015.Rdata")
#save(autors_table_clean,file="pubmed_Authors_Molecular_clock_1970_2015.Rdata")

#load("/Users/sobel/Desktop/Literature mining/pubmed_data_Molecular_clock_1970_2015.Rdata")
#load("/Users/sobel/Desktop/Literature mining/pubmed_Authors_Molecular_clock_1970_2015.Rdata")
...
ddata_sub= subset(ddata, N>2 & sum_IF>10,select=c("Author","sum_IF"))
...
ddata_sub= subset(ddata, N>10,select=c("Author","N"))
...
pubmed_data_sel=pubmed_data[which(pubmed_data$Cited>10),]
pubmed_data_sel=pubmed_data_sel[which(as.numeric(as.character(pubmed_data$ImpactFactor))>5),]
pubmed_data_sel=pubmed_data_sel[which(!is.na(pubmed_data_sel$ImpactFactor)),]

pubmed_data_sel=pubmed_data_sel[-which(pubmed_data_sel$ImpactFactor=="Not Available"),]
pubmed_data_sel$ImpactFactor= as.numeric(as.character(pubmed_data_sel$ImpactFactor))
pubmed_data_sel$ImpactFactor= as.numeric(as.character(pubmed_data_sel$ImpactFactor))
pubmed_data_sel$Journal=as.character(pubmed_data_sel$Journal)
...
pubmed_data_sel=orderBy(~ImpactFactor, pubmed_data_sel)

pubmed_data_sel=orderBy(~-Cited, pubmed_data_sel)

write.csv(pubmed_data_sel[1:40,], file = "pubmed_data_sel_top40.csv",row.names=FALSE,sep="\t")
